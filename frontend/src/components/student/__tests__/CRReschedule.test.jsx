...
describe('CRReschedule Basic Tests', () => {
  ...
  beforeEach(() => {
    ...
    global.fetch = jest.fn(() =>
      Promise.resolve({
        json: () => Promise.resolve({ success: true, message: 'Rescheduling Request Sent to Faculty & Admin for approval.', requests: [] })
      })
    );
  });
  ...
  test('triggers alert on valid form submission', async () => {
    ...
    const secureKeyInput = screen.getByPlaceholderText(/Enter CR Secure Key/i);
    fireEvent.change(secureKeyInput, { target: { value: '123' } });
